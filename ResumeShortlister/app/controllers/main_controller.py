...
@main_bp.route('/bootstrap.min.css')
def bootstrap_css_legacy_path():
    """Serve Bootstrap CSS for legacy templates that request root asset path."""
    return send_from_directory(current_app.static_folder, 'css/bootstrap.min.css')

@main_bp.route('/bootstrap.min.css.map')
def bootstrap_css_map_legacy_path():
    """Return an empty sourcemap response to avoid noisy 404 logs."""
